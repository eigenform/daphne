...
impl Dap { 
    ...
    /// Send a command packet, then immediately read a response packet.
    fn send_raw(&self, pkt: &DapPacketBuf) -> Result<DapPacketBuf> { 
        self.send_pkt(pkt)?;
        self.read_pkt()
    }
